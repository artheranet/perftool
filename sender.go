...
func NewSender(url string, name string) *Sender {
	s := &Sender{
		url:       url,
...
func (s *Sender) sendTx(tx *Transaction, client *ethclient.Client) (err error) {
	...
	if err == nil {
		...
		s.Log.Debug("tx sending ok", "hash", txHash, "dsc", tx.Dsc)
		return
	}
...
func (s *Sender) readReceipts(n *big.Int, client *ethclient.Client) (err error) {
	...
	for index, tx := range blk.Transactions() {
		...
		s.Log.Debug("new receipt", "block", n, "index", index, "tx", txHash)
	}
