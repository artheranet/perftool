...
var (
	...
	app = flags.NewApp("the transactions generator CLI")
...
func makeFakenetAccs(ctx *cli.Context) error {
	...
	for i := accsOffset; i < (accsOffset + accsCount); i++ {
		key := fake.FakeKey(idx.ValidatorID(i))
...
func initAccsBalances(ctx *cli.Context) error {
	...
	generator := NewBalancesGenerator(cfg, keyStore, amount, "InitBalance")
...
func generateCalls(ctx *cli.Context) error {
	...
	generator := NewCallsGenerator(cfg, keyStore, "CallsGen")
	err = generate(generator, maxTps)
	return err
}
...
func generateTransfers(ctx *cli.Context) error {
	...
	generator := NewTransfersGenerator(cfg, keyStore, "TransfersGen")
	err = generate(generator, maxTps)
	return err
}
